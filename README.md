...
我們剛才引入了 Margin Loss，過擬合的風險應該會降低一些，因此我們可以嘗試將模型容量加大。這裡我們使用 `resnet50_pretrained_arcface.yaml` 作為設定檔，將 backbone 改為 ResNet-50。
...
除了加大模型之外，我們也可以嘗試將輸入影像的大小加大，這樣可以讓模型學到更多的細節特徵。
...
加大輸入影像的大小後，模型在第 5 個 epoch 時就達到 80% 以上的準確度，最後是在 174 個 epoch 時，test-set 的準確率達到了 81.21%，比之前的 61.76% 提升了 19.45%。

明明只是把原本 $32 \times 32$ 的影像放大到 $224 \times 224$，卻能讓模型學到更多細節，關鍵原因有三個：
...
首先，計算量大幅增加，訓練時間從原本的 3 分鐘左右，變成大約 4.5 小時（基於 RTX 4090）。其次，前五個 epoch 就達到 80% 的準確率，這表示模型在訓練初期就已經學到了大部分的特徵，但後續的提升幅度卻不大，這可能是因為該資料集的資訊差不多就這樣了，後續幾百個 epoch 的訓練無法再找出更有價值的資訊，因此最終也只能再提升一個百分點左右。
